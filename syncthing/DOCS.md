...
Configuration is done via [Syncthing's web UI](/68413af6_syncthing) (embedded into Home Assistant). First start the app from the [*Info* tab](/config/app/68413af6_syncthing/info) and then click `OPEN WEB UI`.
...
| Path             | Description                                                                                                                                                |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/addon_configs` | Configuration of all Home Assistant apps (formerly known as "add-ons").                                                                                    |
| `/addons`        | Local Home Assistant apps.                                                                                                                                 |
...
| **`/media`**     | Media files to be shared between apps and Home Assistant. See below for a possible way to automatically mount external storage devices under this path.    |
| ***`/share`***   | Data to be shared between apps and Home Assistant. This is the default path for synced folders.                                                            |
| `/ssl`           | TLS/SSL certificates.                                                                                                                                      |

Only the above directories are mapped into the app container. If you put synced folders under any other directory (like `/root` or `/mnt`), the synced data is deleted on app restart. We recommend to put synced folders under either **`/share`** or **`/media`**. Both of these directories are intended to be shared between apps, so you can access synced folders via the [Terminal & SSH](/config/app/core_ssh/info) app, for example.

Furthermore, note that

- a backup of the Syncthing app will include its `/data` directory where its internal state is stored. This folder can take up several GiB of data.
...
If you want to monitor the Syncthing app via Home Assistant's [**Syncthing** integration](https://www.home-assistant.io/integrations/syncthing/), you need to expose Syncthing's web UI to the (local) network rather than only to the [Home Assistant Supervisor](https://developers.home-assistant.io/docs/supervisor).

To do so, go to the app's [*Configuration* tab](/config/app/68413af6_syncthing/config), toggle `Show disabled ports` and enter a port number in the field labelled *Web frontend (not needed with Ingress)* (`8384`, as per the default, is fine). Click `Save`, switch to the [*Info* tab](/config/app/68413af6_syncthing/info) and click `Restart`.
